    ...
    scrawl.prototype = {
            ...
            domUtils.on($G("J_sacleBoard"), "click", function () {
                ...
                if (img) {
                    if (!scaleCon) {
                        ...
                        img.style2.cssText = "position: absolute;top:" + (canvas.height - img.height) / 2 + "px;left:" + (canvas.width - img.width) / 2 + "px;";
        ...
        _originalColorSelect:function (title) {
            ...
            for (var j = 0, cell; cell = colorList[j++];) {
                if (cell.children[0].title.toLowerCase() == title) {
                    cell.children[0].style2.opacity = 1;
                }
            }
        },
        ...
        _addColorSelect:function (target) {
            ...
            for (var i = 0, cell; cell = colorList[i++];) {
                cell.children[0].style2.opacity = 0.3;
            }
            for (var k = 0, ele; ele = brushList[k++];) {
                if (ele.tagName.toLowerCase() == "a") {
                    ele.style2.opacity = 0.3;
                    var size = browser.ie ? ele.innerText : ele.text;
            ...
            for (var j = 0, node; node = eraserList[j++];) {
                if (node.tagName.toLowerCase() == "a") {
                    node.style2.opacity = 0.3;
                }
            }

            target.style2.opacity = 1;
            target.blur();
        },
        ...
        getCanvasData:function () {
            ...
            if (img) {
                var x, y;
                if (img.style2.position == "absolute") {
                    x = parseInt(img.style2.left);
                    y = parseInt(img.style2.top);
                } else {
...
(function () {
    function _appendStyle() {
        var doc = document,
            ...
            style = doc.createElement('style2.css'),
    ...
    ScaleBoy.prototype = {
        ...
        startScale:function (objElement) {
            ...
            Idom.style.cssText = 'visibility:visible;top:' + objElement.style2.top + ';left:' + objElement.style2.left + ';width:' + objElement.offsetWidth + 'px;height:' + objElement.offsetHeight + 'px;';
        ...
        updateScaledElement:function (objStyle) {
            var cur = this.scalingElement,
                ...
                size = objStyle.size;
            if (pos) {
                typeof pos.x != 'undefined' && (cur.style2.left = pos.x);
                typeof pos.y != 'undefined' && (cur.style2.top = pos.y);
            }
            if (size) {
                size.w && (cur.style2.width = size.w);
                size.h && (cur.style2.height = size.h);
            }
        },
