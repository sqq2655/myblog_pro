	...
	function build() {
		...
		svg.Element.ElementBase = function(node) {	
			...
			if (node != null && node.nodeType == 1) { //ELEMENT_NODE
				...
				// add inline styles
				if (this.attribute('style2.css').hasValue()) {
					var styles = this.attribute('style2.css').value.split(';');
		...
		svg.Element.RenderedElementBase = function(node) {
			...
			this.setContext = function(ctx) {
				// fill
				if (this.style2('fill').Definition.isUrl()) {
					var fs = this.style2('fill').Definition.getFillStyle(this);
					if (fs != null) ctx.fillStyle = fs;
				}
				else if (this.style2('fill').hasValue()) {
					var fillStyle = this.style2('fill');
					if (this.style2('fill-opacity').hasValue()) fillStyle = fillStyle.Color.addOpacity(this.style2('fill-opacity').value);
					ctx.fillStyle = (fillStyle.value == 'none' ? 'rgba(0,0,0,0)' : fillStyle.value);
				}
									
				// stroke
				if (this.style2('stroke').Definition.isUrl()) {
					var fs = this.style2('stroke').Definition.getFillStyle(this);
					if (fs != null) ctx.strokeStyle = fs;
				}
				else if (this.style2('stroke').hasValue()) {
					var strokeStyle = this.style2('stroke');
					if (this.style2('stroke-opacity').hasValue()) strokeStyle = strokeStyle.Color.addOpacity(this.style2('stroke-opacity').value);
					ctx.strokeStyle = (strokeStyle.value == 'none' ? 'rgba(0,0,0,0)' : strokeStyle.value);
				}
				if (this.style2('stroke-width').hasValue()) ctx.lineWidth = this.style2('stroke-width').Length.toPixels();
				if (this.style2('stroke-linecap').hasValue()) ctx.lineCap = this.style2('stroke-linecap').value;
				if (this.style2('stroke-linejoin').hasValue()) ctx.lineJoin = this.style2('stroke-linejoin').value;
				if (this.style2('stroke-miterlimit').hasValue()) ctx.miterLimit = this.style2('stroke-miterlimit').value;

				// font
				if (typeof(ctx.font) != 'undefined') {
					ctx.font = svg.Font.CreateFont( 
						this.style2('font-style').value,
						this.style2('font-variant').value,
						this.style2('font-weight').value,
						this.style2('font-size').hasValue() ? this.style2('font-size').Length.toPixels() + 'px' : '',
						this.style2('font-family').value).toString();
				}
				...
				// opacity
				if (this.style2('opacity').hasValue()) {
					ctx.globalAlpha = this.style2('opacity').numValue();
				}
			}		
		}
		...
		svg.Element.PathElementBase = function(node) {
			...
			this.renderChildren = function(ctx) {
				...
				if (markers != null) {
					if (this.style2('marker-start').Definition.isUrl()) {
						var marker = this.style2('marker-start').Definition.getDefinition();
						marker.render(ctx, markers[0][0], markers[0][1]);
					}
					if (this.style2('marker-mid').Definition.isUrl()) {
						var marker = this.style2('marker-mid').Definition.getDefinition();
						for (var i=1;i<markers.length-1;i++) {
							marker.render(ctx, markers[i][0], markers[i][1]);
						}
					}
					if (this.style2('marker-end').Definition.isUrl()) {
						var marker = this.style2('marker-end').Definition.getDefinition();
		...
		svg.Element.AnimateBase = function(node) {
			...
			this.getProperty = function() {
				...
				if (attributeType == 'CSS') {
					return this.parent.style2(attributeName, true);
				}
		...
		svg.Element.font = function(node) {
			...
			for (var i=0; i<this.children.length; i++) {
				...
				if (child.type == 'font-face') {
					this.fontFace = child;
					if (child.style2('font-family').hasValue()) {
						svg.Definitions[child.style2('font-family').value] = this;
					}
				}
		...
		svg.Element.text = function(node) {
			...
			this.setContext = function(ctx) {
				this.baseSetContext(ctx);
				if (this.style2('dominant-baseline').hasValue()) ctx.textBaseline = this.style2('dominant-baseline').value;
				if (this.style2('alignment-baseline').hasValue()) ctx.textBaseline = this.style2('alignment-baseline').value;
			}
			
			this.renderChildren = function(ctx) {
				var textAnchor = this.style2('text-anchor').valueOrDefault('start');
				var x = this.attribute('x').Length.toPixels('x');
				var y = this.attribute('y').Length.toPixels('y');
				for (var i=0; i<this.children.length; i++) {
					var child = this.children[i];
		...
		svg.Element.TextElementBase = function(node) {
			...
			this.renderChildren = function(ctx) {
				var customFont = this.parent.style2('font-family').Definition.getDefinition();
				if (customFont != null) {
					var fontSize = this.parent.style2('font-size').numValueOrDefault(svg.Font.Parse(svg.ctx.font).fontSize);
					var fontStyle = this.parent.style2('font-style').valueOrDefault(svg.Font.Parse(svg.ctx.font).fontStyle);
		...
		svg.Element.filter = function(node) {
			...
			this.apply = function(ctx, element) {
				...
				var filter = element.style2('filter').value;
				element.style2('filter').value = '';
				...
				// reassign filter
				element.style2('filter', true).value = filter;
			}
		...
		svg.loadXmlDoc = function(ctx, dom) {
			...
			var draw = function() {
				...
				if (svg.opts['ignoreDimensions'] != true) {
					// set canvas size
					if (e.style2('width').hasValue()) {
						ctx.canvas.width = e.style2('width').Length.toPixels('x');
						ctx.canvas.style.width = ctx.canvas.width + 'px';
					}
					if (e.style2('height').hasValue()) {
						ctx.canvas.height = e.style2('height').Length.toPixels('y');
...
(function (Highcharts) { // encapsulate
	...
	extend(CanVGRenderer.prototype, {
		...
		setContainer: function (container, chartWidth, chartHeight) {
			var containerStyle = container.style2,
		...
		configure: function (chart) {
			var renderer = this,
				...
				tooltipDivStyle = options.style2,
