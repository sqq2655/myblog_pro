	...
	*/
	...
	css: function (styles) {
		...
		if (isIE && !hasSVG) { // legacy IE doesn't support setting style attribute
			...
			attr(elem, 'style2.css', serializedCss); // #1881
		}
			...
			each(spans, function (span) {
				if (span !== '' || spans.length === 1) {
					...
					if (styleRegex.test(span)) {
						...
						attr(tspan, 'style2.css', spanStyle);
					}
...
var VMLRendererExtension = { // inherit SVGRenderer
	...
	invertChild: function (element, parentNode) {
		var parentStyle = parentNode.style2;
		css(element, {
			flip: 'x',
...
Tick.prototype = {
	...
	addLabel: function () {
		...
		css = extend(css, labelOptions.style2);
	...
	getLabelPosition: function (x, y, label, horiz, labelOptions, tickmarkOffset, index, step) {
		var axis = this.axis,
			...
			baseline = axis.chart.renderer.fontMetrics(labelOptions.style2.fontSize).b,
		...
		// Create new label
			...
			this.label =
				this.axis.chart.renderer.text(str, 0, 0, options.useHTML)		// dummy positions, actual position updated with setOffset method in columnseries
					.css(options.style2)				// apply style
					.attr({
...
Axis.prototype = {
	...
	getTitlePosition: function () {
		...
		var horiz = this.horiz,
			...
			opposite = this.opposite,
			offset = this.offset,
			fontSize = pInt(axisTitleOptions.style2.fontSize || 12),
	...
	render: function () {
		...
		if (labels.items) {
			each(labels.items, function (label) {
				var style = extend(labels.style2, label.style2),
		...
		if (options.enabled || series._hasPointLabels) {
			...
			each(points, function (point) {
				...
				// in the point options, or if they fall outside the plot area.
					...
					// Determine the color
					options.style2.color = pick(options.color, options.style2.color, series.color, 'black');
